function RecycleSymbol({ label, sublabel, color, size = 80 }) {
  const labelBoxW = sublabel ? 350 : 300
  const labelBoxH = sublabel ? 160 : 120

  return (
    <svg
      width={size}
      height={size}
      viewBox="-10 0 790 740"
      xmlns="http://www.w3.org/2000/svg"
    >
      {/* 재활용 마크 — Wikimedia Commons public domain 기반 */}
      <g fill={color} stroke={color} strokeWidth="8" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* 중앙 라벨 */}
      <rect
        x={385 - labelBoxW / 2} y={sublabel ? 290 : 310}
        width={labelBoxW} height={labelBoxH}
        rx="12" fill={color}
      />
      {sublabel ? (
        <>
          <text x="385" y="365" textAnchor="middle" fill="white"
            fontSize={label.length > 3 ? "68" : "82"} fontWeight="900"
            fontFamily="'Apple SD Gothic Neo','Malgun Gothic',sans-serif">
            {label}
          </text>
          <text x="385" y="430" textAnchor="middle" fill="rgba(255,255,255,0.92)"
        ...
        <text x="385" y="385" textAnchor="middle" fill="white"
          fontSize={label.length > 3 ? "68" : "82"} fontWeight="900"
          fontFamily="'Apple SD Gothic Neo','Malgun Gothic',sans-serif">
          {label}
        </text>
